...
export default function SideMenuBar({ open, setOpen }) {
    ...
    return (
        <div>
            {
                open && (
                    <div className="absolute top-0 left-0 bg-white border border-[#eee] z-50 w-1/2 py-12 px-8 min-h-svh">
                        ...
                        <ul className="flex flex-col justify-center gap-8 mt-6">
                            {
                                navLinks.map((item, i) => (
                                    <Link onClick={()=> {item?.drop ? setDropOpen(!dropOpen) :null}} key={i} to={item.link}>
                                        ...
                                        {item?.drop &&<SideMenuDropdown setDropOpen={setDropOpen} dropOpen={dropOpen} />}
                                    </Link>
                                ))
                            }
